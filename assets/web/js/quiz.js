...
function update_view(){
	...
	$.ajax({
		url: $url,
		type: 'post',
		data: {'quiz_id':$quiz_id, 'viewed' : $viewed},
...
function moveQuestion(s, q) {
	if (s >= 0 && s < quiz.length && q >= 0 && q < quiz[s]['array_question'].length) {
		...
		if (quiz[s]['array_question'][q]['image'].length > 0) {
			html += '<img src="../../uploads/img/' + quiz_id + '/' + quiz[s]['array_question'][q]['image'] + '" ';
...
// bắt sự kiện để chạy file âm thanh


function SoundFX($arg) {
	if ($arg == 0) {
		...
		document.getElementById("DoubleKill").play();
	}
	else if ($arg == 3) {
		document.getElementById("TripleKill").play();
	}
	else if ($arg == 4 ) {
		document.getElementById("QuadraKill").play();
	}
	else if ($arg == 5 ) {
		document.getElementById("PentaKill").play();
	}
	else if ($arg > 5 ) {
		document.getElementById("Legendary").play();
	}
}
