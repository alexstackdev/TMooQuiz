...
class Index extends Public_Controller {
    ...
    public function category($slug = "")
    {
    	...
    	$this->data['quiz_new'] = $this->db->query("SELECT * FROM quiz JOIN user ON quiz.user_id=user.user_id WHERE category_id=$cat_id ORDER BY quiz_id DESC")->result();
    	$this->data['quiz_info'] = array(
    ...
    {
    	$this->data['qs'] = $this->db->query("SELECT * FROM quiz JOIN user ON quiz.user_id=user.user_id JOIN category ON quiz.category_id = category.category_id WHERE quiz_id =' $id  ' AND quiz_slug='$slug' ")->row_array();    	
    ...
    public function search(){
        ...
        {
            $this->data['result'] = $this->db->query("SELECT * FROM quiz JOIN user ON quiz.user_id=user.user_id JOIN category ON quiz.category_id = category.category_id WHERE (title LIKE '%$key%') OR quiz_id='$key' ORDER BY viewed DESC ")->result();
        }
        $this->data['quiz_info'] = array(
            'title'     => 'Tìm kiếm : '.$key,
